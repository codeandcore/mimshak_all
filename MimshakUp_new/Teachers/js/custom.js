$(document).ready(function () {
    ...
    if(tabName){
        if(tabName == "z")
        {
            ...
            $(".list li a[data-type="+tabName+"]").addClass("active");
        }
        else{
        ...
        $(".setA").hide();
        $(".setx").show();
        $('.tab1 li:first-child a').addClass('active');
        
        var maxHeight = -1;
        $('.wall_out').each(function() {
            if ($(this).height() > maxHeight) {
                maxHeight = $(this).height();
            }
        });
        $('.wall_out').height(maxHeight);
    }

    $(".tab1 a:not(.redirect_tab)").click(function () {
        var type = $(this).data("type");
        $(".setA").hide();
        $(".set" + type).fadeIn();
        $(".tab1 a").removeClass("active");
...
$(document).ready(function() {
    $('.side_anc .editingmode').click(function(event){
        event.stopPropagation();
    });
});
